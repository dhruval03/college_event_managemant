...
const DataTable = ({ data }) => {
    // Check if data is an array, if not set filteredData to an empty array
    const filteredData = Array.isArray(data) ? data.filter(row => row.isStudent && row.status === "Accept") : [];

    return (
        <div className="table-responsive">
            <table className="table table-bordered table-striped custom-table student-table-custom-size">
                <thead>
                    <tr>
                        <th>Sr.No.</th>
                        <th>Faculty Name</th>
                    ...
                    {filteredData.length > 0 ? (
                        filteredData.map((row, index) => (
                            <tr key={index}>
                                ...
                                <td>{row.Subject}</td>
                                <td>{row.topicName}</td>
                                <td hidden>{row.isStudent ? "true" : "false"}</td>
                            </tr>
                        ))
                    ) : (
                        <tr>
                            <td colSpan="11">No Session For Selected Date.</td>
                        </tr>
                    )}
                </tbody>
            </table>
